...
조회(상영 시간) - 예매하기 전
  - 사용자는 영화, 극장, 날짜를 선택해서 상영 시간을 확인한다.
  - 사용자는 현재 상영 중이거나, 상영 예정인 영화를 선택한다.
  - 사용자는 현재 운영 중인 극장을 선택한다.
  - 사용자는 상영 시간이 등록된 날짜를 선택한다.
  - 사용자는 영화, 극장, 날짜를 선택하여 상영 시간 및 관과 전체 좌석 수/예매 가능 좌석 수를 확인한다.
조회(영화 상세 조회)
  - 사용자는 영화를 선택하여 상세 조회를 한다.
  - 상세 정보는 감독, 배우, 장르, 기본 정보, 개봉일, 영화 설명, 트레일러, 스틸컷이 있다.
  - 감독은 영화를 연출한 감독으로 등록된 감독 모두를 조회한다.
  - 배우는 영화에 참여한 배우로, 영화를 등록할 때 참여한 배우를 등록한 순으로 조회한다.
  - 장르는 영화 장르로, 등록된 장르 모두를 조회한다.
  - 기본 정보는 관람 나이, 런닝 시간(분), 제작 국가가 조회된다.
...
상영관 관리
  ...
  - 극장을 선택하고, 상영관 번호, 총 좌석 수, 좌석 정보를 입력하여 등록한다.
  - 상영관 번호는 해당 극장의 총 상영관 수보다 작거나 같아야 한다.
  - 좌석 정보는 행은 영어 대문자로, 열은 숫자로 표현하여 해당 상영관의 모든 좌석을 등록한다.
  - 등록된 좌석 정보에 맞게 총 좌석 수를 저장한다.
  - 좌석은 상황에 따라 예매가 불가능하다.
상영 시간 관리
  - 관리자는 상영 시간을 등록, 수정, 삭제한다.
  - 상영 시간은 상영관 정보, 영화, 날짜, 상영 시간을 입력하여 등록한다.
  - 상영관 정보는 극장을 선택하면 상영관이 조회되고, 조회된 상영관 중 하나를 선택한다.
  - 영화는 등록된 영화 중 하나를 선택해서 등록한다.
  - 상영 시간은 10분 단위로 등록한다.
요금 관리
  - 관리자는 요금을 수정한다.
  - 요금은 성인, 청소년, 조조 할인으로 구분된다.
  - 청소년은 성인 요금의 80%이다.
