...
교수 등록/수정/삭제
  ...
  - 교번은 직접 입력하지 않고, 첫 자리는 P, 취임년도 4자리, 전공 코드 3자리, 취임순(같은 년도) 3자리를 이용하여 구성된다.
지도 교수 등록/수정/삭제
  - 학생은 전공 교수 중 한 명이 지도 교수로 배정된다.
과목 등록/수정/삭제
  - 과목 코드, 과목명, 이수 학점, 시간을 입력하여 등록한다.
  - 과목 코드는 분류 코드 3자리, 순서 3자리로 구성된다.
  - 분류 코드는 교직(000), 교양(001), 전공별로 할당된 전공 코드를 사용한다.
  - 이수 학점은 이수 시 인정되는 학점으로, 0 ~ 4학점까지 이다.
...
수강 신청/취소
  ...
  - 한 학생은 같은 기간에 같은 강의를 여러 번 수강 신청 할 수 없다.
성적 등록/수정/삭제
