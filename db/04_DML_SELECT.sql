...
SELF JOIN
 - 하나의 테이블로 JOIN을 하는 경우
*/

/*
- 테이블A가 테이블B가 참조되고 있으면 테이블B에는 외래키, 테이블A에는 기본키로 연결이 되어 있었을 때 JOIN을 사용
	=> 학생 테이블이 성적 테이블에 참조되고 있으면 성적 테이블에 외래키로 SC_ST_KEY, 학생 테이블에 ST_KEY로 연결이 되어 있음
INNER JOIN
...
# 뷰의 장점 : 1. 재사용 2. 가독성 3. 쿼리의 단순화 4. 보안 강화 => 특정 컬럼이나 데이터만 보여줄 수 있음
/*
...
# 모든 과목의 평균을 조회하는 쿼리
SELECT 
    SUBJECT.*, IFNULL(AVG(SC_SCORE), 0) 평균
FROM
    SUBJECT
LEFT JOIN
...
# 모든 학생의 평균을 조회하는 쿼리. 성적이 없는 경우는 0이 나오도록 처리
SELECT 
    STUDENT.*, IFNULL(AVG(SC_SCORE), 0) 평균
FROM STUDENT
...
# 2. JOIN보다 가독성이 좋음
# 3. 속도를 향상 시킬 수 있음
#	홍길동이라는 사람이 KH학원에 다니는 아들을 찾으러 왔음
#	일반 JOIN : 모든 강의장에 있는 학생들 중에 성이 홍씨인 남자를 찾고, 그 중에 아들을 찾음
#	서브쿼리 : 각 강의장에서 성이 홍씨이고 남자인 학생들을 찾고, 그 중에 아들을 찾음
# 서브쿼리 적용 위치
# 1. SELECT문에서 속성 위치
# 2. SELECT문에서 FROM 다음에 나오는 테이블 위치
...
SELECT 
	RANK() OVER(ORDER BY 평균 DESC, 국어평균 DESC, 영어평균 DESC, 수학평균 DESC) 순위, T.*
FROM 
	(SELECT 
		ST_GRADE 학년, ST_CLASS 반, ST_NUM 번호, ST_NAME 이름, 
...
# 2학년 등수를 조회하는 쿼리(평균).
SELECT 
	RANK() OVER(ORDER BY 평균 DESC) 순위, T.*
FROM 
	(SELECT 
		ST_GRADE 학년, ST_CLASS 반, ST_NUM 번호, ST_NAME 이름, IFNULL(AVG(SC_SCORE), 0) 평균
	FROM SCORE
