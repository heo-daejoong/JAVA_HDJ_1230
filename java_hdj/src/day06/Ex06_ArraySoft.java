	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < arr.length - 1; i++) {
			for(int j = 0; j < arr.length - 1; j++) {
				if(arr[j] > arr[j+1]) { //부등호에 따라 내림차순, 오름차순 설정 가능
					int tmp = arr[j];
					arr[j] = arr[j+1];
					arr[j+1] = tmp;
				}
		...
		/* Arrays.sort(배열) : 오름차순으로 정렬. Dual-Pivot Quicksort 알고리즘 이용. 시간복잡도 O(nlogn)*/
		Arrays.sort(arr2);
		// Array.thString(배열) : 배열을 문자열로 변환. [값, 값, ....]
