...
public class Ex02_Inheritance {
	...
	public static void main(String[] args) {
		/* kiaCar 객체를 만들 때, Car클래스의 멤버변수가 먼저 생성되고
		 * Car클래스의 멤버변수(power, speed)가 먼저 생성되고,
		 * Car클래스의 생성자로 초기화 됨
		 * KiaCar 클래스에서 선언한 멤버변수를 생성하고
		 * KiaCar 클래스의 생성자로 초기화 됨
		 * */
		KiaCar kia = new KiaCar();
		//kia.power = true; //에러발생 : 다른 클래스에서 private를 사용할 수 없다.
		kia.turnOn();
		System.out.println(kia.isPower());
		//kia.speed = 10; 
		//에러발생 : 자식 클래스가 아니고, 다른 패키지에 있는 클래스에서 사용할 수 있다.
...
class Hyundai extends Car{
	...
	public void test() {
		//다른 패키지이지만 자식 클래스이기 때문에 speed를 사용할 수 있다.
		speed = 0;
