...
public class ChatClient {
	...
	public void send() {
		...
		try(ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream())) {
			...
			System.out.println("[자유롭게 채팅하시고 종료하려면 " + EXIT + "를 입력하세요.]");
			while(true) {
	...
	public void run() {
		...
		//이때 둘 중 하나는 쓰레드를 제거. 밑에 있는 메소드의 쓰레드를 제거하면 됨
		//제거하는 이유? 제거를 안하면 두 메소드가 쓰레드로 실행되기 때문에
		//run메소드가 종료되고, 그러면 메인 메뉴가 채팅이 끝나지 않은 상태에서 종료될 수 있기 때문에.
		receive();
		send();
